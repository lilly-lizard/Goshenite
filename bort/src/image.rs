...
impl Image {
    pub fn new(
        ...
        let image_memory_reqs =
            unsafe { device.inner().get_image_memory_requirements(image_handle) };
        ...
        let image_memory = unsafe {
            device
                .inner()
                .allocate_memory(&image_allocate_info, ALLOCATION_CALLBACK)
        }
...
impl Drop for Image {
    fn drop(&mut self) {
        unsafe {
            self.device
                .inner()
                .free_memory(self.image_memory, ALLOCATION_CALLBACK);
            self.device
                .inner()
